...
import com.infeez.mock.MockServerTest.StubModel
import com.infeez.mock.converter.ConverterFactory
import com.infeez.mock.dsl.context.MockServerContext
import com.infeez.mock.dsl.customMockServer
import com.infeez.mock.dsl.mock
import com.infeez.mock.dsl.nettyMockServer
import com.infeez.mock.dsl.okHttpMockServer
import com.infeez.mock.extensions.mock
...
class MockServerV1Test {
    ...
    @get:Rule
    val nettyServer = nettyMockServer {
    ...
    private fun servers(): List<Server> {
        ...
        return listOf(OkHttpServer(configuration), NettyServer(configuration))
    }
    ...
    fun `replace mock test`() {
        val mock1 = mock({ path { eq("/mock/url") } }) {
            body("$defaultResponse 1")
        }

        val mock2 = mock({ path { eq("/mock/url") } }) {
    ...
    fun `remove mock test`() {
        val mock = mock({ path { eq("/mock/url") } }) {
            body(defaultResponse)
        }
    ...
    fun `change mock response body test`() = runServers {
        val mock = com.infeez.mock.dsl.mock({ path { eq("/mock/url") } }) {
            body(json {
                "a" to "a"
                "b" to 1
                "c" to 2L
    ...
    fun `copy mock test`() = runServers {
        val mock1 = com.infeez.mock.dsl.mock({ path { eq("/mock/url") } }) {
            code(201)
            headers {
                "a" to "123"
            }
    ...
    fun `body param converter one url multiple time test`() = runServers {
        mock({ path { eq("/some/path") } and body { bodyMarch<StubModel> { a == "a" } } }) {
            body(("response string a"))
        }
        mock({ path { eq("/some/path") } and body { bodyMarch<StubModel> { a == "b" } } }) {
            body(("response string b"))
        }
        mock({ path { eq("/some/path") } and body { bodyMarch<StubModel> { a == "c" } } }) {
    ...
    fun `bodyEq test`() = runServers {
        mock({ path { eq("/some/path") } and body { bodyEq<StubModel>("""{"a":"a","b":1,"c":2,"d":3.0}""") } }) {
            body(("response string a"))
        }
